...
extension CaveAddViewController {
    func saveGoalForTest(){
        let startDate = Calendar.current.date(byAdding: .day, value: -97, to: Date())!
        let lastDate = Calendar.current.date(byAdding: .day, value: 99, to: startDate)!
        let encoder = JSONEncoder()
           ...
           let userID = defaults.value(forKey: keyForDf.crrUser) as? String
        {
            ...
            let usersFailAllowInput = failAllowOutput.selectedSegmentIndex
            let newGoal = GoalStruct(userID: userID, goalID: startDateForDB, startDate: startDate, endDate: lastDate, failAllowance: usersFailAllowInput, description: description, numOfDays: 100, completed: false, goalAchieved: false, numOfSuccess: 97, numOfFail: 0, shared: false)
